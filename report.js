...
var task = cron.schedule('0  0  *  *  *', () => {
	getJSON('http://localhost', function(error, response){
		...
		profit_day = ((total_profit - last_result.total_profit).toFixed(8) * -1);
		profit_day = parseFloat(profit_day);
			
		profit_day_percentage = ((total_profit_percentage - last_result.total_profit_percentage).toFixed(2) * -1);
...
var server=http.createServer((function(request,response)
{
	...
	response.write('</head><body><p>&nbsp;<h1 align="center"> Stable Coin - Report</h1></p> <table width="238" border="1" class="tabelaum"> <tr> <td colspan="2" align="center"><strong>Balances</strong></td> </tr>');
	...
	response.write('<tr><td><strong>Total:</strong></td><td>&nbsp;'+(result_report.saldo_USDT + result_report.saldo_PAX + result_report.saldo_TUSD + result_report.saldo_USDC +  result_report.saldo_USDS + result_report.saldo_USDSB) +'</td></tr></table>');
